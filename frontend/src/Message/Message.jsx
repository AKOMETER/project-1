...
function Message() {
  ...
  const handleSelectChange = (event) => {
    ...
    if (selectedComponent) {
      ...
      setApiurl1((prevApiurl1) => {
        return `${
          config.baseUrl
        }sent-messages/images?template_format=${findFormat(selectedComponent)}`;
      });
  ...
  //handle submit for individual message
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!select || !apiurl1) {
      toast("Please Select a Template for Messaging", toastOptions);
      return;
    }
  ...
  const handleSubmit2 = () => {
    ...
    axios
      .post(`${config.baseUrl}custom-message/`, data, { headers: headers })
      .then((response) => {
        setPhoneNumberInput("");
        setCustommessage("");
        setTimeout(() => {
      ...
      <div className="flex-1 p-5 h-screen overflow-y-scroll">
        ...
        <div className=" flex gap-10 mt-4">
          <div
            className={
              messagingbox
                ? "cursor-pointer select-none py-1 px-2 text-lg bg-[#064A42] text-white rounded-md hover:shadow-xl"
                : "cursor-pointer select-none py-1 px-2 text-lg bg-[white] rounded-md hover:shadow-xl"
            }
            onClick={() => {
              setMessagingbox(true);
              setPhoneNumberInput("");
            }}
          >
            ...
            <div className=" flex justify-between px-10 mt-7 max-md:flex-col-reverse max-lg:flex-col-reverse max-sm:px-2">
              <div className=" flex flex-col gap-1 w-full">
                ...
                <div
                  ...
                  className="select-none cursor-pointer flex justify-center items-center bg-[#064A42] max-w-max text-white py-1 px-2 rounded-lg uppercase font-bold text-xs tracking-widest	"
                >
                  <img
              ...
              <div>
                ...
                <div className="bg-[#262d31] text-gray-300 rounded-tr-lg  rounded-bl-lg rounded-br-lg mb-4 px-4 py-2 mt-4 w-[300px] overflow-hidden">
                  ...
                  {componentData &&
                    findFormat(componentData) === "document" && (
